...
import { VALID, VIEW_MODE, VALID_TEXT } from '@vs-common/utils/const/enum.js'
import { isArray } from '@vs-common/utils/type/index.js'
import { isEmpty as isEmptyArray } from '@vs-common/utils/array/index.js'
import { toBranch } from '@vs-common/utils/package/index.js'
...
export const useRunning = ({ attrs, slots, emits, props, name }) => {
  ...
  const onClick = (data, node, treeNode, pointerEvent) => {
    ...
    emits('view:mode', {
      ...
      viewMode: node.key === FLAG.NEW ? VIEW_MODE.INSERT : VIEW_MODE.DETAILS
    })
  }
  ...
  const onAppend = async node => {
    ...
    emits('view:mode', { data: dataset.newNode, key: FLAG.NEW, viewMode: VIEW_MODE.INSERT })
  }
  ...
  const onEdit = (node, data) => {
    const { children, ...residue } = data
    emits('view:mode', { data: residue, key: node.key, viewMode: VIEW_MODE.UPDATE })
  }
   ...
   */
  const onSubmit = () => {
    if (isArray(dataset.checkedNodes, dataset.checkedKeys)) {
      if (isEmptyArray(dataset.checkedNodes) && isEmptyArray(dataset.checkedKeys)) {
    ...
    then(response => {
      if (response.success) {
        if (!isEmptyArray(response.data)) {
          ...
          dataset.dataSource = toBranch(response.data).nesting
